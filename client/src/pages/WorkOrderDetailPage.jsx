...
export default function WorkOrderDetailPage() {
  ...
  const handleStatusChange = async (newStatus) => {
    try {
      ...
      setWo(prev => ({ ...prev, status: res.data?.status || newStatus, completed_at: res.data?.completed_at }));
    } catch (err) {
  ...
  const currentStatus = wo.status || 'scheduled';
        ...
        <main className="flex-1 p-8 space-y-6 max-w-4xl mx-auto">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <h1 className="text-xl font-extrabold text-white tracking-tight mt-1">{wo.title || 'Work Order Detail'}</h1>
            </div>
          </div>
          ...
          <div className="glass-panel p-6 rounded-2xl border border-industrial-border space-y-6">
            <div className="flex items-center justify-between border-b border-industrial-border pb-4">
              <div>
                ...
                <div className="text-base font-bold text-accent-cyan capitalize">{currentStatus.replace(/_/g, ' ')}</div>
              </div>
            ...
            <div>
              ...
              <p className="text-xs text-gray-200 bg-industrial-900 p-4 rounded-xl border border-industrial-border whitespace-pre-wrap leading-relaxed">
                {wo.description || 'No work description recorded.'}
              </p>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4 text-xs">
              <div className="bg-industrial-900/80 p-4 rounded-xl border border-industrial-border">
                ...
                <span className="font-bold text-white">{wo.asset?.name || wo.asset_id || 'Plant Machinery'}</span>
              </div>
